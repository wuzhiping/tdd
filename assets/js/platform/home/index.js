...
app.controller("indexCtrl",['$scope','baseService',function(scope,baseService){
	...
	scope.logout = function(systemId){
        ...
        scope.home = "逺東新企業風控戰情平台";
        scope.homeUrl =  "dev/ERM.html"; // "sys/workbenchPanel/myWorkbench.html";

	scope.closeAll = function(){
		scope.openedMenu = [{id:"indexpage",active:"active",name:scope.home,noclose:true,url:scope.homeUrl}]; 
	}
	scope.cloaseOther = function(){
		var array = [{id:"indexpage",active:"active",name:scope.home,noclose:true,url:scope.homeUrl}];
		for(var i=0,item;item = scope.openedMenu[i++];){
			if(item.active === "active" && item.name !==scope.home){
				array.push(item);
			}
		}
	...
	// url特殊处理
	window.setTimeout(function(){
		$("#indexpageiframe").attr("src",scope.homeUrl);
	},10)
	var userInfoTab = {
			id:"userInfo",
			name:"个人信息",
